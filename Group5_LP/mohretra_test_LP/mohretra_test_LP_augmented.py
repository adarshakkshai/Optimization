...
#Matt's additions
    ...
    D_squared = S_inv@X	 	#D^2 = S^(-1)X
    #Define spd matrix to use for the augmented form of the step direction linear system
    spd = D_squared@A_tran
    spd = A@spd
    #compute the cholesky factorization of spd
    G = np.linalg.cholesky(spd)
    G_tran = G.transpose()
#End of Matt's additions
#----------------------------------------------------------------------------------------------

    # making matrix of equation 14.30
#I am trying to do the algorithm without building this matrix
#    m_11 = np.zeros((5, 5))
#    m_12 = A_tran
#    m_13 = np.identity((5))
...
#----------------------------------------------------------------------------------------------
#Matt's attempt at	computing 'delta_x_affine', 'delta_lambda_affine', 'delta_s_affine' by using equation 14.30
    #setup the RHS of equation for delta_lambda_affine
    rhs = D_squared@r_c
    rhs = rhs + x
    rhs = A@rhs
    rhs = (-1) * (r_b + rhs)
    #solve GG'(delta_lambda_affine) = rhs , where G is lower triangular
    y = scipy.linalg.solve_triangular(G,rhs)
    delta_lambda_affine = scipy.linalg.solve_triangular(G_tran,y)
    print('D_squared')
    print(D_squared)

    #calculate delta_s_affine
    delta_s_affine = A_tran@delta_lambda_affine
    delta_s_affine = (-1) * (r_c + delta_s_affine)
    
    #calculate delta_x_affine
    delta_x_affine = D_squared@delta_s_affine
    delta_x_affine = (-1) * (x + delta_x_affine)
#End of Matt's attempt 
...
#-----------------------------------------------------------------------------------------------
#Matt's attempt at calculating delta_x,delta_lambda,delta_s using the augmented form 
    #calculate the RHS of equation for delta_lambda
    e = np.ones((n,1))
    rhs2 = delta_S_affine.dot(e)
    rhs2 = delta_X_affine.dot(rhs2)
    t = sigma * mu
    rhs2 = t*e
    ...
    rhs = A@rhs
    rhs = -1 * r_b + rhs
    #solve GG'(delta_lambda) triangular system
    y = scipy.linalg.solve_triangular(G,rhs)
    delta_lambda = scipy.linalg.solve_triangular(G_tran,y)
     
    #calculate delta_s
    delta_s = A_tran@delta_lambda
    delta_s = (-1) * (r_c + delta_s)
    
    #calculate delta_x
    delta_x = rhs2 - x	#rhs2 came from the calculations of delta_lambda
    v = D_squared@delta_s
    delta_x = delta_x - v
#End of Matt's attempt 
#-----------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------
#Matt's attempt at calculating 'alpha_k_primal' and 'alpha_k_dual' from equation (14.38)
